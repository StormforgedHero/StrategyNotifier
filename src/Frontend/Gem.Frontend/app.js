(() => {
  ...
  const LANG_STORAGE_KEY = 'sn_language';
  const I18N_PATH = './i18n/';
  const SUPPORTED_LANGS = ['en', 'pl'];
  const SUBSCRIBE_FORM_URL = (window.SN_SITE_CONFIG && window.SN_SITE_CONFIG.subscribeUrl) || '';
  ...
  const DEFAULT_I18N = {
    ...
    statusIdle: 'Loading...',
    statusManifestMissing: 'Manifest not found. Signals may need regeneration by the maintainer.',
    ...
    manifestHelp: 'Signals must be regenerated by the maintainer when missing.',
  ...
  function applyFooterLanguage(lang) {
    ...
    const copy = FOOTER_COPY[safeLang];
    const privacyLink = document.querySelector('[data-footer="privacy-link"]');
    const subscribeLink = document.querySelector('[data-footer="subscribe-link"]');
    if (privacyLink) {
      privacyLink.textContent = copy.privacy;
      privacyLink.href = `./privacy.html?lang=${safeLang}`;
    }
    if (subscribeLink) {
      if (SUBSCRIBE_FORM_URL) {
        subscribeLink.textContent = copy.subscribe;
        subscribeLink.href = SUBSCRIBE_FORM_URL;
